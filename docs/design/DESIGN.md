...
| ------------------------------- | ---------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Run control                     | `RunControl`                                               | Prompt/PRD intake, local folder selector, adapter selector, model + effort controls, review-mode toggle, worker-cap (1–20, default 10), start/cancel/retry                                                                                                                                                                                                                                                                                                                                                             |
...
| Package handoff (U8)            | `PackageHandoff`                                           | Packaged repo path, handoff + provenance references, packaging commit, and artifact confidence once `package.created` exists; makes the local-first contract visible — package and provenance persist even when deploy pauses/fails (R30)                                                                                                                                                                                                                                                                              |
...
| Status headline (U5)            | `BlueprintLanes` headline block                            | One plain-words line: what the focused run is doing now + how many items need the operator, the items one expand away. Needs-you = union of open interventions and unpaired pending reviews (a stage review paired with its open intervention is ONE decision). When the count is zero, an explicit designed "nothing needs you in this run" state renders — never absence of content                                                                                                                                  |
| Stage pipeline (U5)             | `BlueprintLanes` lane cards                                | The eight lanes ARE the pipeline. A current-stage marker (text label + `aria-current="step"`, never color alone) sits on the furthest lane with live work, falling back to the furthest completed stage; retries render within their lane (repair), never as pipeline regression. Non-current lane cards collapse by default and expand on demand to the EXISTING panels (ResearchBrief, SupervisorPanel, WorkerBoard, PackageHandoff, DeployStatus) or projection-faithful gate/repair rows — no ledger-fidelity loss |
| Live run list (U5)              | `useRunList` + `RunStrip`/`RunBoard`                       | GET /api/runs polled on the shared cadence: runs started from CLI/MCP appear without reload; auto-focus ONLY when no run is focused; a focused run archived from another surface refocuses the newest visible run or shows the empty state with an "archived — open in run history" notice                                                                                                                                                                                                                             |
| History view (U6)               | `RunBoard`                                                 | Archive-aware history host: include-archived toggle, unarchive action, replay links into the existing run detail view; replaces the ephemeral "Clear view" state with the real archive lifecycle                                                                                                                                                                                                                                                                                                                       |
| Session lifecycle controls (U6) | `FactoryCommandBar`                                        | New Session (arm/confirm pattern; the confirm names active runs and warns when deployed artifacts remain live) and Factory Reset (visually and semantically separated; typed confirmation rendering the server's pre-flight enumeration; reset-generation change renders a forced-reload banner)                                                                                                                                                                                                                       |
