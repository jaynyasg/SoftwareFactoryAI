...
async function derive(
  store: EventStore,
  generatedFiles: readonly string[] = ['package.json', 'app/page.tsx'],
) {
...
describe('deriveRunProvenance', () => {
  it('derives every section from ledger evidence and embeds the confidence', async () => {
    ...
    await append(store, {
      type: 'gate.passed',
      payload: { gate: 'lint', summary: 'clean', stage: 'post_run' },
    });
    await append(store, {
      type: 'gate.passed',
  ...
  it('counts the LATEST outcome per gate so a repaired gate is not double-counted', async () => {
    ...
    await append(store, {
      type: 'gate.failed',
      payload: { gate: 'unit-test', reason: 'red', stage: 'post_ticket' },
    });
    await append(store, {
      type: 'gate.passed',
      payload: { gate: 'unit-test', summary: 'green after repair', stage: 'post_ticket' },
    });
