...
description: >-
  Run the local Software Factory from Codex (Windows/PowerShell). Submit a prompt
  or PRD and get back a real run with a ticket DAG, gate/preview/deploy evidence,
  a packaged repo, and a handoff summary — all produced by the same local backend
...
The script lives at `skills/codex/scripts/software-factory.ps1` (PowerShell).
