...
add(
  ...
  'warn',
  {
    action: 'Authenticate the local Codex CLI before running.',
    reason: 'No adapter session found.',
  },
...
add(
  ...
  'info',
  { metric: 'cpu', value: 0.42, unit: 'ratio', status: 'ok' },
  { actor: SYSTEM },
);
add(
  'operator.health_sample',
  'info',
  { metric: 'memory', value: 0.55, unit: 'ratio', status: 'ok' },
  { actor: SYSTEM },
);
add(
  'operator.health_sample',
  'info',
  { metric: 'queue_wait', value: 180, unit: 'ms', status: 'ok' },
  { actor: SYSTEM },
);
...
add(
  ...
  'info',
  { moduleId: 'scaffold-app', version: '1' },
  { ...ticket('scaffold'), actor: GENOME },
);
...
add(
  'worker.progress',
  'info',
  { message: 'scaffold files written', percent: 100 },
...
add(
  ...
  { gate: 'unit-test', reason: '2 unit tests failing in data-model' },
  {
    ...ticket('data-model'),
    actor: GATE,
    evidence: [{ label: 'test output', ref: 'gates/data-model/unit-test.log' }],
  },
);
add(
  'worker.retry',
  'warn',
  { attempt: 1, reason: 'unit-test gate failed' },
  { ...ticket('data-model'), actor: WORKER },
);
add('gate.started', 'info', { gate: 'unit-test' }, { ...ticket('data-model'), actor: GATE });
add(
  'gate.passed',
  'success',
  { gate: 'unit-test', summary: 'all unit tests passing' },
  { ...ticket('data-model'), actor: GATE },
);
add(
  'worker.completed',
  'success',
  { summary: 'data model + migrations ready' },
  { ...ticket('data-model'), actor: WORKER },
);
...
add(
  'gate.passed',
  'success',
  { gate: 'typecheck', summary: 'tsc clean' },
  { ...ticket('api-contract'), actor: GATE },
);
add(
  'worker.completed',
  'success',
...
add(
  'operator.health_sample',
  'warn',
  { metric: 'cpu', value: 0.91, unit: 'ratio', status: 'degraded' },
  { actor: SYSTEM },
);
...
add(
  'worker.progress',
  'info',
  { message: 'request flow wired', percent: 80 },
  { ...ticket('marketplace-ui'), actor: WORKER },
);
add(
  'worker.completed',
  'success',
  { summary: 'request flow ready' },
  { ...ticket('marketplace-ui'), actor: WORKER },
);
add(
  'worker.completed',
  'success',
  { summary: 'AI brief generation ready (deterministic fallback)' },
  { ...ticket('ai-brief'), actor: WORKER },
);
add(
  'worker.completed',
  'success',
  { summary: 'provider proposals ready' },
  { ...ticket('provider-proposals'), actor: WORKER },
);
...
add(
  'gate.passed',
  'success',
  { gate: 'unit-test', summary: 'suite green' },
  { ...ticket('tests'), actor: GATE },
);
add('gate.started', 'info', { gate: 'secret-scan' }, { ...ticket('tests'), actor: GATE });
add(
  'gate.passed',
  'success',
  { gate: 'secret-scan', summary: 'no secrets found' },
  { ...ticket('tests'), actor: GATE },
);
add(
  'worker.completed',
  'success',
  { summary: 'quality gates authored + green' },
  { ...ticket('tests'), actor: WORKER },
);
...
add(
  ...
  { ...ticket('preview'), actor: WORKER },
);
add('gate.started', 'info', { gate: 'preview-health' }, { ...ticket('preview'), actor: GATE });
add(
  'gate.passed',
  'success',
  { gate: 'preview-health', summary: 'health 200 OK' },
  { ...ticket('preview'), actor: GATE },
);
add(
  'worker.completed',
  'success',
...
add(
  ...
  'success',
  {
    repoPath: 'generated/ai-services-marketplace',
    handoffRef: 'HANDOFF.md',
    summary: 'repo packaged with provenance',
  },
...
add(
  ...
  'info',
  {
    ...
    kind: 'provenance',
    path: 'generated/ai-services-marketplace/PROVENANCE.json',
  },
  { ...ticket('package') },
);
add(
...
add(
  'worker.completed',
  'success',
  { summary: 'package + provenance ready' },
  { ...ticket('package'), actor: WORKER },
);
...
add(
  ...
  {
    actor: SUPERVISOR,
    subject: { kind: 'ticket', id: 'deploy', version: 1 },
    ticketId: 'deploy',
...
add(
  ...
  'success',
  {
    riskTier: 'high',
    decision: 'approved',
    rationale: 'Two operators approved the hosted deploy.',
  },
...
add(
  ...
  { reason: 'render.yaml missing healthCheckPath.' },
  {
    actor: DEPLOY,
    ...ticket('deploy'),
    evidence: [{ label: 'render.yaml', ref: 'generated/ai-services-marketplace/render.yaml' }],
  },
);
add(
  ...
  { reason: 'Render build failed: exit code 1.' },
  {
    actor: DEPLOY,
    ...ticket('deploy'),
    evidence: [{ label: 'render build log', ref: 'deploy/render/build.log' }],
  },
);
add(
  ...
  { reason: 'prisma migrate deploy failed: relation "ServiceRequest" already exists.' },
  {
    actor: DEPLOY,
    ...ticket('deploy'),
    evidence: [{ label: 'migrate log', ref: 'deploy/render/migrate.log' }],
  },
);
...
add(
  ...
  { reason: 'Hosted health returned 503 after 5 attempts.' },
  {
    actor: DEPLOY,
    ...ticket('deploy'),
    evidence: [{ label: 'health log', ref: 'deploy/render/health.log' }],
  },
);
add('deploy.health_pending', 'info', {}, { actor: DEPLOY, ...ticket('deploy') });
add(
...
add(
  'worker.completed',
  'success',
  { summary: 'hosted deploy live + healthy' },
  { ...ticket('deploy'), actor: WORKER },
);

add(
  'operator.health_sample',
  'info',
  { metric: 'cpu', value: 0.38, unit: 'ratio', status: 'ok' },
  { actor: SYSTEM },
);
add(
  'run.completed',
  'success',
  { summary: 'AI Services Marketplace generated, gated, previewed, packaged, and deployed.' },
  { actor: SUPERVISOR },
);
